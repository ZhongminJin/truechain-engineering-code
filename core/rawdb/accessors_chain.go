...
// ReadHeadBlockHash retrieves the hash of the current canonical head block.
func ReadHeadRewardNumber(db etruedb.Reader) uint64 {
...
// WriteHeadBlockHash stores the head block's hash.
func WriteHeadRewardNumber(db etruedb.Writer, number uint64) {
...
func ReadLastBlockHash(db etruedb.Reader) common.Hash {
...
func WriteLastBlockHash(db etruedb.Writer, hash common.Hash) {
...
func ReadStateGcBR(db etruedb.Reader) uint64 {
...
func WriteStateGcBR(db etruedb.Writer, count uint64) {
...
// reporting correct numbers across restarts.
func ReadFastTrieProgress(db etruedb.Reader) uint64 {
...
// retrieving it across restarts.
func WriteFastTrieProgress(db etruedb.Writer, count uint64) {
...
func ReadHeader(db etruedb.Reader, hash common.Hash, number uint64) *types.Header {
...
// to-number mapping.
func WriteHeader(db etruedb.Writer, header *types.Header) {
...
func ReadBody(db etruedb.Reader, hash common.Hash, number uint64) *types.Body {
...
func WriteBody(db etruedb.Writer, hash common.Hash, number uint64, body *types.Body) {
...
// to a block.
func HasReceipts(db etruedb.Reader, hash common.Hash, number uint64) bool {
...
func ReadRawReceipts(db etruedb.Reader, hash common.Hash, number uint64) types.Receipts {
	// Retrieve the flattened receipt slicept)(r).setStatus(sto
...
func ReadReceipts(db etruedb.Reader, hash common.Hash, number uint64, config *params.ChainConfig) types.Receipts {
...
func WriteReceipts(db etruedb.Writer, hash common.Hash, number uint64, receipts types.Receipts) {
...
func ReadBlock(db etruedb.Reader, hash common.Hash, number uint64) *types.Block {
...
// canonical hash can be stored in the database but the body data not (yet).
func ReadSnapBlock(db etruedb.Reader, hash common.Hash, number uint64) *types.Block {
	header := ReadHeader(db, hash, number)
	if header == nil {
		return nil
	}
...
func WriteBlock(db etruedb.Writer, block *types.Block) {
...
// ReadHeaderRLP retrieves a block header in its raw RLP database encoding.
func ReadBlockRewardRLP(db etruedb.Reader, number uint64) rlp.RawValue {
	data, _ := db.Get(blockRewardKey(number))
	return data
}

func ReadBlockReward(db etruedb.Reader, number uint64) *types.BlockReward {
...
func WriteBlockReward(db etruedb.Writer, block *types.BlockReward) {
...
func FindCommonAncestor(db etruedb.Reader, a, b *types.Header) *types.Header {
...
// ReadTd retrieves a block's total difficulty corresponding to the hash.
func ReadCommitteeInfo(db etruedb.Reader, hash common.Hash, number uint64) []*types.CommitteeMember {
...
// WriteTd stores the total difficulty of a block into the database.
func WriteCommitteeInfo(db etruedb.Writer, hash common.Hash, number uint64, info []*types.CommitteeMember) {
